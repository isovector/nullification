...
import           Data.Ecstasy.Types (Update (..), Component, ComponentType(..), StorageType(..))
...
instance (GGetEntity f f')
    => GGetEntity (M1 i c f) (M1 i' c' f') where
  gGetEntity (M1 a) e = M1 $ gGetEntity a e
  {-# INLINE gGetEntity #-}

instance (GGetEntity a c, GGetEntity b d)
    => GGetEntity (a :*: b) (c :*: d) where
...
class GetField (c :: ComponentType) where
  getField
      :: Component 'WorldOf c a
      -> Int
      -> Maybe a

instance GetField 'Field where
  getField c i = I.lookup i c
...
instance GSetEntity (K1 i (Update a)) (K1 i' (IntMap a)) where
  ...
  gSetEntity (K1 (Modify f)) e (K1 b) = K1 $ I.alter (fmap f) e b
  {-# INLINE gSetEntity #-}

instance (GSetEntity f f')
    => GSetEntity (M1 i c f) (M1 i' c' f') where
  gSetEntity (M1 a) e (M1 b) = M1 $ gSetEntity a e b
  {-# INLINE gSetEntity #-}

instance (GSetEntity a c, GSetEntity b d)
    => GSetEntity (a :*: b) (c :*: d) where
